...
## TestFlight: first App Store Connect submission

- Brought the project onto the same EAS Build + Submit + Update pattern as the user's other app (FamFit): `expo-updates` installed with `runtimeVersion: { policy: "appVersion" }` and an `updates.url` pointing at this project's EAS Update branch, so pure JS/content changes (like the trail/map work above) can ship via `eas update --branch <branch>` to installed builds without a new TestFlight build. `eas.json` build profiles (`development`/`preview`/`production`) each map to their own update channel; `production` explicitly requests `distribution: "store"`.
- Confirmed via `eas build:list`/`eas.json` on the user's other project (FamFit) that both projects share the same Expo account (`swocevol`) and Apple Developer team (`NM3YWJFH8B`, David Keim Individual) -- user confirmed this directly.
- User created the App Store Connect app entry for `com.swocevol.familyscriptureapp` (Apple ID `6799738543`) and it's now wired into `eas.json`'s `submit.production.ios.ascAppId`, so `eas submit` doesn't need to prompt for it.
- **First production build + submit required two rounds of one-time interactive credential setup** that couldn't be done through a non-interactive CLI call -- both are normal EAS behavior, not bugs:
  1. `eas build --profile production` failed non-interactively with "Distribution Certificate is not validated for non-interactive builds" -- needed the user to run it once in their own terminal to authenticate with Apple directly (session cookie had expired) and confirm certificate validation. Two-factor prompt included, as expected for any fresh Apple ID login.
  2. `eas submit` failed non-interactively with "App Store Connect API Keys cannot be set up in --non-interactive mode" -- a *different* credential (the App Store Connect API Key used for uploads, distinct from the build-signing certificate) that also needed one-time interactive setup. The user ran it once, chose to let EAS auto-generate a new API key via the now-authenticated Apple session, and it succeeded.
  - Both credentials are now stored server-side on EAS (the distribution cert was already there; the new API Key is named "[Expo] EAS Submit qyiiI372sP", Key ID `8426VM635G`) -- **future `eas build --profile production` and `eas submit` calls should work fully non-interactively**, no more terminal hand-offs expected for routine releases.
- First production build: build number 4, uploaded successfully, App Store Connect now processing it (~5-10 min) before it's available in TestFlight.
- **Not yet done** (App Store Connect UI, user's side): create the external "Family" TestFlight group (mirroring FamFit's setup) and add testers by email; fill out the one-time Test Information for that group; submit for Apple's Beta App Review (only required for external testing, not internal).
